...
![Docker CI Tools](screenshots/Containers-DockerNative.png)
![Docker CI Tools](screenshots/Containers-VirtualBox.png)
![Docker CI Tools](screenshots/Containers-Jenkins.png)
![Docker CI Tools](screenshots/Containers-C.png)
...
* /opt/sonarqube via __sudo mkdir /opt/sonarqube__
* /opt/selenium via __sudo mkdir /opt/selenium__
...
The pre-loaded Jenkins jobs are stored in the \jobs folder.  There is a generatePlugins.groovy and a updatePlugins.groovy script.
...
Sonar Qube seems to have issue.  Working this. Just started with last iteration. The Qube is there but the Redis is not parsing the request or something.  Need to start up just this container and debug.
